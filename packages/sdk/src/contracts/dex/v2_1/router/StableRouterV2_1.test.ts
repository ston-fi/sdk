...
describe("StableRouterV2_1", () => {
  beforeAll(setup);

  describe("version", () => {
    it("should have expected static value", () => {
      expect(StableRouterV2_1.version).toBe(DEX_VERSION.v2_1);
    });
  });

  describe("dexType", () => {
    it("should have expected static value", () => {
      expect(StableRouterV2_1.dexType).toBe(DEX_TYPE.Stable);
    });
  });
  ...
  describe("getPool", () => {
    it("should return Pool instance with expected version", async () => {
      ...
      const contract = provider.open(StableRouterV2_1.create(ROUTER_ADDRESS));

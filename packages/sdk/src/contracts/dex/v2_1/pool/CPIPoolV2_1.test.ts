...
describe("CPIPoolV2_1", () => {
  beforeAll(setup);

  describe("version", () => {
    it("should have expected static value", () => {
      expect(CPIPoolV2_1.version).toBe(DEX_VERSION.v2_1);
    });
  });

  describe("dexType", () => {
    it("should have expected static value", () => {
      expect(CPIPoolV2_1.dexType).toBe(DEX_TYPE.CPI);
    });
  });
  ...
  describe("getLpAccount", () => {
    it("should return LpAccount instance with expected version", async () => {
      ...
      const contract = provider.open(CPIPoolV2_1.create(POOL_ADDRESS));

...
describe("StablePoolV2_2", () => {
  beforeAll(setup);

  describe("version", () => {
    it("should have expected static value", () => {
      expect(StablePoolV2_2.version).toBe(DEX_VERSION.v2_2);
    });
  });

  describe("dexType", () => {
    it("should have expected static value", () => {
      expect(StablePoolV2_2.dexType).toBe(DEX_TYPE.Stable);
    });
  });
  ...
  describe("getLpAccount", () => {
    it("should return LpAccount instance with expected version", async () => {
      ...
      const contract = provider.open(StablePoolV2_2.create(POOL_ADDRESS));

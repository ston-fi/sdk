...
describe("StableRouterV2_2", () => {
  beforeAll(setup);

  describe("version", () => {
    it("should have expected static value", () => {
      expect(StableRouterV2_2.version).toBe(DEX_VERSION.v2_2);
    });
  });

  describe("dexType", () => {
    it("should have expected static value", () => {
      expect(StableRouterV2_2.dexType).toBe(DEX_TYPE.Stable);
    });
  });
  ...
  describe("getPool", () => {
    it("should return Pool instance with expected version", async () => {
      ...
      const contract = provider.open(StableRouterV2_2.create(ROUTER_ADDRESS));
